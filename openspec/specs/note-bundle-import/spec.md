...
Import a brain-pack zip (produced by `/export` in another brain) into this vault: validate safely, triage collisions, place notes into the target taxonomy, stamp provenance, update indexes, and report — with at most two user prompts per run. TBD: expand.
