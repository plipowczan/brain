...
Namespace the vault's workflow surface: slash-command wrappers live under `/b:*` and vault-specific skills carry the `brain-` prefix, in both the brain repo and kb-template, with all cross-references kept consistent and the breaking rename documented for template subscribers. TBD: expand.
