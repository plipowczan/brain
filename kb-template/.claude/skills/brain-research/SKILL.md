...
This workspace lives **outside** the published Quartz content (it's under `content/_raw/`) — working files won't be deployed. Only the final `report.md` is later promoted to `content/_raw/inbox/` by `/b:research-report` for `/b:ingest` to classify into a topic folder.
...
## Follow-up Commands
- `/b:research-add-items` - Supplement items
- `/b:research-add-fields` - Supplement fields
- `/b:research-deep` - Start deep research
