...
`/b:lint` diagnoses; `/b:curate` treats. This skill is action-oriented but every
...
- `dead_link` starts False. Only run live checks on the **flagged subset** — notes
  ...
  `python .claude/skills/brain-curate/scripts/curate_links.py <url> ...`. A `"dead"`
...
- **archive** → move the file to `content/_graveyard/` preserving its folder name as
  ...
  inbound wikilinks (delegate to `/b:refactor` link-repair).
- **merge** → delegate to `/b:refactor` (merge into the superseding note; it repairs
  wikilinks).
- **refresh** → delegate to `/b:enhance` (re-verify and update the note in place).
- **keep** → no action.
...
- `/b:reindex` — rebuilds indexes if missing or stale. Run before curate if needed.
- `/b:lint` — passive diagnosis. `/b:refactor` — merge + wikilink repair. `/b:enhance` — refresh.
