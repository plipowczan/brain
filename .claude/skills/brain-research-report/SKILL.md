...
After `report.md` is generated in the workspace, create a brain-ready copy in `content/_raw/inbox/` so `/b:ingest` can classify it into the proper topic folder.
...
- Suggest: "Run `/b:ingest` to classify this into the proper topic folder (likely `AI/`, `BUSINESS/`, etc.) and update indexes."
- Workspace files (`outline.yaml`, `fields.yaml`, `results/*.json`, `generate_report.py`, original `report.md`) stay in `content/_raw/research-workspaces/{topic_slug}/` for future `/b:research-add-items` or `/b:research-add-fields` runs.
