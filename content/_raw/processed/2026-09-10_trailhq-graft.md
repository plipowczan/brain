---
title: "trailhq/Graft: Turbocharge Claude Code, Cursor, Codex, Gemini & every coding agent: faster, cheaper, with contextual understanding specific to your codebase."
source: "https://github.com/trailhq/Graft"
author:
published:
created: 2026-09-10
description: "Turbocharge Claude Code, Cursor, Codex, Gemini & every coding agent: faster, cheaper, with contextual understanding specific to your codebase. - trailhq/Graft"
tags:
  - "clippings"
---
[![[5b85e1d4dfd9d1113f21d8bd6776e332_MD5.png]]](https://github.com/trailhq/Graft/blob/main/assets/graft-hero.png)

### Turbocharge Claude Code, Cursor, Codex, Gemini & every coding agent: faster, cheaper, with contextual understanding specific to your codebase.

[![[5540ed112d2df18cdcc668b4b1e576c2_MD5.svg]]](https://trendshift.io/repositories/92209?utm_source=trendshift-badge&utm_medium=badge&utm_campaign=badge-trendshift-92209)
...
[![[eed35d2a680af0058e91b7186fc17609_MD5.gif]]](https://github.com/trailhq/Graft/blob/main/assets/graft-comparison-demo.gif)
...
[![[01786a83d47e398f7f40a93d86f8f234_MD5.png]]](https://github.com/trailhq/Graft/blob/main/assets/graft-terminal.png)
...
[![[ed160933f3405dfa9c20c7343fd03505_MD5.gif]]](https://github.com/trailhq/Graft/blob/main/assets/graft-site-act-demo.gif)
...
![[5ded8227e78f3e8df8f68a872f56a684_MD5.png]]
...
<sub>Correctness over all instances; tokens, cost and calls over the instances both arms resolved, for a like-for-like comparison. Official SWE-bench Verified images and official <code>swebench</code> 4.1.0 grader, native x86_64.</sub>
...
- **Full-fidelity** — hand-written extractors with scope-aware, cross-file call and import resolution: **TypeScript / JavaScript** (incl. JSX & TSX), **Python**, **Go**, **Java**, **Kotlin**, **PHP**, **Swift** (classes, structs, enums, actors, protocols; extension members attach to the extended type), **R** (`.R` /`.r` — plain functions, S3/S4/R6 classes and methods, roxygen `@export`, `library()` / `source()` imports).
...
Twenty-three languages in total. A file whose language isn't listed is skipped, not indexed. Adding a broad-tier language is a small contribution — see [CREDITS.md](https://github.com/trailhq/Graft/blob/main/CREDITS.md) for the folks who added the current set.
...
- **Anonymous usage stats** — the only network calls are the LLM requests you configured, a daily npm version check, and one batched usage ping. The ping carries buckets and fixed labels only: never your code, file paths, repo name, symbols, queries, or error messages. [`TELEMETRY.md`](https://github.com/trailhq/Graft/blob/main/TELEMETRY.md) is the complete list and `graft telemetry debug` prints exactly what your machine would send. Turn it off with `graft telemetry disable`, `DO_NOT_TRACK=1`, or by unchecking the box in `graft init`; it is off in CI and in any build from source.

See [`.env.example`](https://github.com/trailhq/Graft/blob/main/.env.example) for the full list of settings (model, base URL, graph directory).
...
[![[6f951e1454679d292cd548c35d8ad2b2_MD5.gif]]](https://github.com/trailhq/Graft/blob/main/assets/graft-hooks-demo.gif)

  
<sub>install → init → hooks keep the graph fresh every session</sub>

[![[fdb45bb2402445ed850b90cef276b9c7_MD5.gif]]](https://github.com/trailhq/Graft/blob/main/assets/graft-hook-blast-radius-demo.gif)

  
<sub>edit a file → blast radius appears inline → graph auto-resyncs → confirmed in <code>graft viz</code></sub>
...
- **A Git superproject with initialized submodules** — submodules stay excluded by default. Run `graft build --follow-submodules` to fold initialized gitlinks into one graph, prefixing child paths (for example, `deps/parser/src/index.ts`) while honoring each submodule's own Git ignore rules. Visible untracked files are included too; uninitialized submodules remain absent until `git submodule update --init` checks them out. The choice is saved in `.graft/config.json`, so later no-flag builds and MCP automatic refreshes behave the same way. Run `graft build --no-follow-submodules` to restore and persist the default boundary.
...
In every layout, narrow to one sub-project with `graft ask "<task>" --in <scope>/` once you know where you're working.
...
[![[141e0083c83b23503925273f452b98c9_MD5.gif]]](https://github.com/trailhq/Graft/blob/main/assets/graft-viz-demo.gif)

  
<sub>search → jump to a node → dependency graph lights up</sub>
